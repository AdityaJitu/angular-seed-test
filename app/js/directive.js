...
directive('dirinsidemodel', [ //model inside
        ...
        }]).directive('parserdirective',['$parse',function($parse){
            return{
                restrict:'AEC',
                link:function(scope, elem ,attr){
                    var getter=$parse('name');
                    var context={name:'Aditya'};

                    scope.name=$parse(attr.add);
                },
                template:'{{name()}}'
            }
    }]).directive('outerdirmodel',[function(){
        return{
            restrict:'AEC',
            scope:{
                value:'=ngModel'
            },
            template:'<button ng-click="decrement()">-</button><input ng-model="value"/><button ng-click="increment()">+</button>',
            link:function(scope, elem, attr){
